...
class Profesor extends CActiveRecord
{
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function  dameNombreProfesor($id)
        {   $nombreProfesor='';
            if(isset($id))
            {
                $profesor = Profesor::model()->findByPk($id);
        ...
        {
            $profesores=array();
            $arrayProfesores=  Profesor::model()->findAll();
           
            
            foreach ($arrayProfesores as  $profesor) 
            {
                $nombreProfesor = $this->dameNombreProfesor($profesor->id);
                $profesores[$profesor->id]=$nombreProfesor;
            }  
            return $profesores;
        }
}
